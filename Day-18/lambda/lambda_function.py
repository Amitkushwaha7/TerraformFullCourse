...
def send_notification(original_key, processed_images, processed_bucket):
    """
    Send SNS notification about processing completion.
    
    Args:
        original_key: Original image filename
    ...
    try:
        ...
        if not sns_topic_arn or sns_topic_arn == '':
            logger.info("SNS notifications not configured, skipping notification")
            return
        
        # Build notification message
        subject = f"✅ Image Processing Complete: {original_key}"
        ...
        message_lines = [
            ...
            f"📦 Processed Bucket: {processed_bucket}",
            f"🖼️  Variants Created: {len(processed_images)}",
            "",
            "📋 PROCESSED VARIANTS:",
            "-" * 60,
        ]
        ...
        message_lines.extend([
            ...
            "-" * 60,
            "📥 DOWNLOAD COMMAND:",
            f"aws s3 cp s3://{processed_bucket}/{original_key.rsplit('/', 1)[0] if '/' in original_key else ''}* ./downloads/ --recursive",
            "",
            "🔍 VIEW IN S3 CONSOLE:",
            f"https://s3.console.aws.amazon.com/s3/buckets/{processed_bucket}",
            "",
            "=" * 60,
