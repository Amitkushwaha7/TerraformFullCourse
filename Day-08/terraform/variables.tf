...
# List type for allowed locations
...
variable "bucket_names" {
  ...
  type = list(string)
  default = [ "my-unique-bucket-name-123456-aws-terraform" ]
}

# Separate list to avoid duplicate bucket creation between count and for_each examples
variable "bucket_names_foreach" {
  description = "List of S3 bucket names for for_each example (distinct from bucket_names)"
  type        = list(string)
  default     = [ "my-unique-bucket-name-123456-aws-terraform-2" ]
}